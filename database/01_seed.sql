...
SELECT 'Kubernetes', 'Platforms'::category, 'Adopt'::ring, 'Cluster', account_ids.id
FROM account_ids WHERE email = 'tech-lead@technology-radar.ch'
UNION ALL
SELECT 'AgroCD', 'Tools'::category, 'Trial'::ring, 'Kubernetes Cluster', account_ids.id
FROM account_ids WHERE email = 'cto@technology-radar.ch'
UNION ALL
SELECT 'GitOps', 'Techniques'::category, 'Assess'::ring, 'Technique for deploying Applications', account_ids.id
FROM account_ids WHERE email = 'tech-lead@technology-radar.ch'
UNION ALL
SELECT 'Web components for SSR', 'Techniques'::category, 'Hold'::ring, 'Server Side Rendering', account_ids.id
FROM account_ids WHERE email = 'tech-lead@technology-radar.ch'
UNION ALL
...
working on different products.', account_ids.id
FROM account_ids WHERE email = 'cto@technology-radar.ch';
